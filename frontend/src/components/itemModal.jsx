      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {items.map((item) => (
            <div 
              key={item.id}
              className={`relative bg-gray-700 p-4 rounded-lg shadow flex flex-col items-center text-center ${rarityClass(item.seltenheit)} `}
            ><div className="relative w-full flex flex-col items-center text-center">
              ...
              {item.im_shop == 1 || item.im_shop === "1" ? (
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  <button onClick={() => handleDestroy(item.id)} className="px-1 py-1 bg-red-600 rounded text-xs">Vernichten</button>
                  <button onClick={() => handleSell(item.id)} className="px-1 py-1 bg-yellow-600 rounded text-xs">Verkaufen</button>
                </div>
              )}
            </div>
